 ...
 * - With ENABLE_STREAM=true (or STREAM_PUBLIC_HOST / STREAM_WEBHOOK_SECRET set):
 *   also emit [stream] binding = "STREAM" so dashboard Stream bindings survive deploy.
...
/** Persist Cloudflare Stream binding when explicitly enabled or Stream env is present. */
export function shouldEnableStreamBinding(
  ...
  // Existing Stream-related config → keep binding (same wipe class as R2).
...
`
  ...
  // Preserve Stream binding so Workers Builds / deploy cannot wipe panel config.
  const streamBlock = shouldEnableStreamBinding(source) ? buildWranglerStreamConfig() : "";
...
max_batch_size = 1
max_batch_timeout = 5
${r2Block}${streamBlock}${analyticsBlock}`;
}
...
export async function main(): Promise<void> {
  ...
  if (r2BucketName) {
    ...
    const streamNote = shouldEnableStreamBinding(envWithDb) ? ", STREAM binding" : "";
    console.log(`✅ Wrote wrangler.toml with R2_BUCKET → ${r2BucketName}, DB → ${dbId}${streamNote}`);
    return;
  }
