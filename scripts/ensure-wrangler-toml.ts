...
`
  ...
  // Preserve VPC Service binding for comment geolocation (same wipe class as R2).
  const ip2regionServiceId = (source.IP2REGION_SERVICE_ID || "").trim();
  const ip2regionBaseUrl = (source.IP2REGION_BASE_URL || "http://ip2region.internal").trim() || "http://ip2region.internal";
  const vpcBlock = ip2regionServiceId
    ? `
[[vpc_services]]
binding = "IP2REGION"
service_id = "${ip2regionServiceId}"
`
    : "";
  const ip2regionVars = ip2regionServiceId
    ? `IP2REGION_BASE_URL = "${ip2regionBaseUrl}"
`
    : "";
