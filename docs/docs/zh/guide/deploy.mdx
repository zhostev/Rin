...
上面两张表是最小可用集。下面是部署流程实际读取的**全部**变量。注意 `Variables` 和 `Secrets` 是同一页面下的**两个不同标签页**，填错地方等于没填。

:::warning 使用 R2 时这一项是硬性必填
`R2_BUCKET_NAME` 是 **Variable** 而不是 Secret。缺失时生产部署会直接失败退出：生成的 `wrangler.toml` 会丢掉 `[[r2_buckets]]`，而 `wrangler deploy` 是整份覆盖远端 Worker 配置的，那会抹掉已有的 R2 绑定，导致 `/api/blob/*` 全部 500。

报错信息里提示的 `ALLOW_DEPLOY_WITHOUT_R2=true` **只对本地部署有效**——GitHub Actions 的 workflow 没有透传这个变量，在仓库设置里配置它不会生效。
:::
...
| 变量名 | 必填 | 默认值 | 说明 |
|--------|------|--------|------|
| `R2_BUCKET_NAME` | 使用 R2 时必填 | 空 | R2 存储桶名称，见上方警告 |
| `S3_ENDPOINT` | 使用 S3 时必填 | 空 | S3/R2 接入点；留空且已填 `R2_BUCKET_NAME` 时会自动推导 |
| `S3_BUCKET` | 使用 S3 时必填 | 空 | 存储桶名称 |
| `S3_ACCESS_HOST` | 否 | 空 | 图片公开访问域名 |
| `S3_FOLDER` | 否 | `images/` | 图片存放目录 |
| `S3_CACHE_FOLDER` | 否 | `cache/` | 缓存目录 |
| `S3_REGION` | 否 | `auto` | 区域 |
| `S3_FORCE_PATH_STYLE` | 否 | `false` | 非 R2 的 S3 兼容服务可能需要 `true` |
| `CACHE_STORAGE_MODE` | 否 | `s3` | 推荐设为 `database`，开箱即用 |
| `FRONTEND_URL` | 否 | 空 | 站点公开地址；sitemap/robots 的绝对链接与 Stream 的 allowedOrigins 都读它 |
| `WEBHOOK_URL` | 否 | 空 | 发布通知 Webhook |
| `RSS_TITLE` | 否 | 空 | RSS 标题 |
| `RSS_DESCRIPTION` | 否 | 空 | RSS 描述 |
| `RSS_ENABLE` | 否 | `false` | 是否启用 RSS |
| `NAME` | 否 | `Rin` | 站点名称 |
| `DESCRIPTION` | 否 | `A lightweight personal blogging system` | 站点描述 |
| `AVATAR` | 否 | 空 | 头像地址 |
| `PAGE_SIZE` | 否 | `5` | 分页大小 |
| `WORKER_NAME` | 否 | `rin-server` | Worker 名称 |
| `DB_NAME` | 否 | `rin` | D1 数据库名称 |
| `ENABLE_STREAM` | 否 | 空 | 设为 `true` 时向生成的 `wrangler.toml` 写入 `[stream]` 绑定 |
| `STREAM_PUBLIC_HOST` | 否 | 空 | 自定义 Stream 播放域名，留空使用 Cloudflare 默认 iframe 地址 |

**Repository Secrets**（同一页面的 Secrets 标签）

| Secret 名称 | 必填 | 说明 |
|------------|------|------|
| `CLOUDFLARE_API_TOKEN` | 是 | 部署令牌 |
| `CLOUDFLARE_ACCOUNT_ID` | 是 | 账户 ID。虽然它不算敏感信息，但 workflow 是按 Secret 读取的，必须放在 Secrets 标签页 |
| `JWT_SECRET` | 是 | JWT 签名密钥 |
| `ADMIN_USERNAME` | 账号密码登录时必填 | 登录用户名 |
| `ADMIN_PASSWORD` | 账号密码登录时必填 | 登录密码 |
| `S3_ACCESS_KEY_ID` | 使用 R2/S3 时必填 | 存储访问密钥 ID |
| `S3_SECRET_ACCESS_KEY` | 使用 R2/S3 时必填 | 存储访问密钥 |
| `RIN_GITHUB_CLIENT_ID` | 否 | GitHub OAuth 登录 |
| `RIN_GITHUB_CLIENT_SECRET` | 否 | GitHub OAuth 登录 |
| `STREAM_API_TOKEN` | 否 | Stream 专用令牌，权限只需 账户 → Stream → Edit。留空时回退到 `CLOUDFLARE_API_TOKEN`（那份令牌则必须自带 Stream:Edit） |
| `STREAM_WEBHOOK_SECRET` | 否 | Stream Webhook 签名密钥，留空则 `/api/media/stream/webhook` 保持关闭，视频状态仅靠轮询刷新 |
...
| Variable 名称 | 说明 | 示例 |
|--------------|------|------|
| `R2_BUCKET_NAME` | **使用 R2 时必填**（Workers Builds / `bun run deploy`）。缺失会省略 `[[r2_buckets]]` 并覆盖掉远端绑定。 | `my-blog-images` |
