...
:::warning 使用 R2 时 `R2_BUCKET_NAME` 是硬性必填
它是 **Variable** 而不是 Secret。缺失时生产部署会直接失败退出：生成的 `wrangler.toml` 会丢掉 `[[r2_buckets]]`，而 `wrangler deploy` 是整份覆盖远端 Worker 配置的，那会抹掉已有的 R2 绑定，导致 `/api/blob/*` 全部 500。
