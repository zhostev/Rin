...
:::warning `R2_BUCKET_NAME` is mandatory when using R2
It is a **Variable**, not a Secret. Without it a production deploy exits before writing `wrangler.toml`: the generated config would omit `[[r2_buckets]]`, and since `wrangler deploy` replaces the remote Worker configuration wholesale, that drops the existing R2 binding and makes every `/api/blob/*` request return 500.
